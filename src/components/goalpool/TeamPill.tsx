  ...
  return (
    <div
      className={cn(
        ...
        selected ? "bg-blue-50 text-slate-950 ring-1 ring-blue-200" : "bg-white hover:bg-slate-50",
        !team && "border border-dashed border-slate-200 bg-slate-50 text-slate-400",
      )}
    >
      <div className="flex min-w-0 items-center gap-2.5">
        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-slate-100 text-lg">
          {team?.flagEmoji ?? "TBD"}
        </span>
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold">{team?.name ?? "Winner advances"}</div>
          <div className="text-[11px] font-medium uppercase tracking-wide text-slate-500">
            {team ? `Seed ${team.seed} · Group ${team.groupName}` : "Pending pick"}
          </div>
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-2">
        {typeof score === "number" ? (
          <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-bold">{score}</span>
        ) : null}
